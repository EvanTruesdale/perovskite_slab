...
K_POINTS {{automatic}}
8 8 1 0 0 0
""".format(_e_ext=e_ext, _calculation=calculation, _prefix=name, _masses=masses, _nat=nat, _ntyp=ntyp, _atomic_positions=atomic_positions, _lattice=lattice)
            
        # Write text to the input file
        input_file = 'in/{_name}_{_e_ext}.{_calculation}.in'.format(_calculation=calculation, _name=name, _e_ext=round(e_ext*1000))
        output_file = 'out/{_name}_{_e_ext}.{_calculation}.out'.format(_calculation=calculation, _name=name, _e_ext=round(e_ext*1000))
...
{_awin}
        ...
        process=subprocess.Popen("mv avg.dat avg_{_e_ext}.dat".format(_e_ext=round(e_ext*1000)), shell=True, stdout=DEVNULL)
        process.wait()
    

    # Run scf with new structure, without electric field as a reference charge density
    e_ext_array = np.arange(0.00, 0.085, 0.005)
    for e_ext in e_ext_array:

        # Remove data from structure file
        try:
            structure_file = open("{_folder}_relaxed/{_name}_{_e_ext}".format(_e_ext=round(e_ext*1000), _folder=folder, _name=name), 'r')
        except:
            continue
        lines = structure_file.readlines()
        structure_file.close()
...
""".format(_prefix=name, _masses=masses, _nat=nat, _ntyp=ntyp, _atomic_positions=atomic_positions, _lattice=lattice)
        ...
        # Write text to the input file
        input_file = 'in/{_name}_{_e_ext}_reference.scf.in'.format(_name=name, _e_ext=round(e_ext*1000))
        output_file = 'out/{_name}_{_e_ext}_reference.scf.out'.format(_name=name, _e_ext=round(e_ext*1000))
        with open(input_file, 'w') as f:
            f.write(text)
...
{_awin}
        ...
        process=subprocess.Popen("mv avg.dat avg_{_e_ext}_reference.dat".format(_e_ext=round(e_ext*1000)), shell=True, stdout=DEVNULL)
        process.wait()
