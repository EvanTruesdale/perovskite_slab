...
for name in structure_files:
    ...
    k_inplane_array = np.array([2., 4., 6., 8., 10., 12., 14.])
    energies = np.zeros(k_inplane_array.shape)    

    # Run through all values of k point
    for i, row in enumerate(k_inplane_array):
        k_inplane = k_inplane_array[i]
...
K_POINTS {{automatic}}
{_k_inplane} {_k_inplane} {_k_inplane} 0 0 0
""".format(_calculation=calculation, _k_inplane=k_inplane, _prefix=name, _masses=masses, _nat=nat, _ntyp=ntyp, _atomic_positions=atomic_positions, _lattice=lattice)
        ...
        if len(energy_steps) != 0:
            ...
            energy = 0
        energies[i] = energy
        print("{k} {e}".format(k=k_inplane, e=energies[i]))
        with open('data/kpoint', 'a') as f:
            f.write("{k} {e}\n".format(k=k_inplane, e=energies[i]))
